...
//延时一定的时钟，有些寄存器在写入后要几个时钟之后，才能再次写入
void delay_ntick(u32 ntick)
{
	u32 ticks;
	for(ticks=ntick;ticks>0;ticks--)
		__nop();
}
  ...
  if (SDIO_MULTIMEDIA_CARD == CardType)
  {
		 ...
		 if (SDIO_BusWide_8b == WideMode)   //2.0 sd不支持8bits
    {
      ...
      return(errorstatus);
		...
		{
			if(WideMode == SDIO_BusWide_4b)
			{
					errorstatus=SDEnWideBus(ENABLE);
			}
			else if(WideMode == SDIO_BusWide_1b)
			{
 			...
 			if(SD_OK==errorstatus)
			{
				...
				//clkcr这个寄存器不能连续写入，必须要几个 sdiclk 时钟周期之后才能继续写入
				delay_ntick(20);
				//必须开启硬件流控，不然会出错
				SDIO->CLKCR|=1<<14;		
				delay_ntick(20);
...
SD_Error SD_Init(int PrePriority,int SubPriority)
{
	...
	//复用IO口配置
	GPIO_Init_AF(SDIO_D0_PIN);
	GPIO_Init_AF(SDIO_D1_PIN);
	GPIO_Init_AF(SDIO_D2_PIN);
	GPIO_Init_AF(SDIO_D3_PIN);
	GPIO_Init_AF(SDIO_SCK_PIN);
	GPIO_Init_AF(SDIO_CMD_PIN);
	GPIO_PinAFConfig(SDIO_D0_PIN_S,GPIO_AF_SDIO);
  GPIO_PinAFConfig(SDIO_D1_PIN_S,GPIO_AF_SDIO);
  GPIO_PinAFConfig(SDIO_D2_PIN_S,GPIO_AF_SDIO);
  GPIO_PinAFConfig(SDIO_D3_PIN_S,GPIO_AF_SDIO);
  GPIO_PinAFConfig(SDIO_SCK_PIN_S,GPIO_AF_SDIO);	
  GPIO_PinAFConfig(SDIO_CMD_PIN_S,GPIO_AF_SDIO);	
...
{		 
  ...
  DMA_InitStructure.DMA_BufferSize = bufsize;//0;//数据传输量 
  ...
  SDIO_DataConfig(&SDIO_DataInitStructure);
	...
	}else if(DeviceMode==SD_DMA_MODE)
	{
   	TransferError=SD_OK;
		...
		if(timeout==0)
		{
  			SD_Init(SDIO_INTPRIO_CFG);	 					//重新初始化SD卡,可以解决写入死机的问题
