...
static void AHRS(Vector3F gyro_raw, Vector3F acc_raw, Vector3F mag_raw, float dt)
{
	...
	//
	hx = mag_raw.x * AHRS_Param.rMat[0][0] + mag_raw.y * AHRS_Param.rMat[0][1] + mag_raw.z * AHRS_Param.rMat[0][2];
	hy = mag_raw.x * AHRS_Param.rMat[1][0] + mag_raw.y * AHRS_Param.rMat[1][1] + mag_raw.z * AHRS_Param.rMat[1][2];
	hz = mag_raw.x * AHRS_Param.rMat[2][0] + mag_raw.y * AHRS_Param.rMat[2][1] + mag_raw.z * AHRS_Param.rMat[2][2];
	bx = sqrt((hx * hx) + (hy * hy));
	bz = hz;

	wx = bx * AHRS_Param.rMat[0][0] + bz * AHRS_Param.rMat[2][0];
	wy = bx * AHRS_Param.rMat[0][1] + bz * AHRS_Param.rMat[2][1];
	wz = bx * AHRS_Param.rMat[0][2] + bz * AHRS_Param.rMat[2][2];


	/*通过G和旋转矩阵的外积得到载体坐标系下的重力加速度表示*/
	/*加速计读取的方向与重力加速计方向的差值，用向量叉乘计算*/
	ex = (acc_raw.y * AHRS_Param.rMat[2][2] - acc_raw.z * AHRS_Param.rMat[2][1]) + mag_raw.y * wz - mag_raw.z * wy;
	ey = (acc_raw.z * AHRS_Param.rMat[2][0] - acc_raw.x * AHRS_Param.rMat[2][2]) + mag_raw.z * wx - mag_raw.x * wz;
	ez = (acc_raw.x * AHRS_Param.rMat[2][1] - acc_raw.y * AHRS_Param.rMat[2][0]) + mag_raw.x * wy - mag_raw.y * wx;
#else
